...
declare const bimap: <E, G, A, B>(f: (e: E) => G, g: (a: A) => B) => (fa: [A, E]) => [B, G], compose: <E, A>(la: [A, E]) => <B>(ab: [B, A]) => [B, E], duplicate: <E, A>(ma: [A, E]) => [[A, E], E], extend: <E, A, B>(f: (fa: [A, E]) => B) => (ma: [A, E]) => [B, E], foldMap: <M>(M: Monoid<M>) => <A>(f: (a: A) => M) => <E>(fa: [A, E]) => M, map: <A, B>(f: (a: A) => B) => <E>(fa: [A, E]) => [B, E], mapLeft: <E, G>(f: (e: E) => G) => <A>(fa: [A, E]) => [A, G], reduce: <A, B>(b: B, f: (b: B, a: A) => B) => <E>(fa: [A, E]) => B, reduceRight: <A, B>(b: B, f: (a: A, b: B) => B) => <E>(fa: [A, E]) => B;
export { 
/**
 * @since 2.0.0
 */
