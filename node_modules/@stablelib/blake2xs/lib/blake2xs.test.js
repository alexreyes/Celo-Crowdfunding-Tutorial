...
var blake2xs_1 = require("./blake2xs");
/* tslint:disable */
var vectors = [
    {
...
/* tslint:enable */
describe("BLAKE2Xs", function () {
    it("should match test vectors", function () {
        vectors.forEach(function (v) {
            var inp = hex.decode(v.in);
            var key = v.key.length > 0 ? hex.decode(v.key) : undefined;
            var h = new blake2xs_1.BLAKE2Xs(v.out.length / 2, { key: key });
            h.update(inp);
            var got = h.digest();
            expect(hex.encode(got, true)).toEqual(v.out);
        });
    });
    it("should generate correct result in chunks", function () {
        var v = vectors[vectors.length - 4];
        var inp = hex.decode(v.in);
        var key = v.key.length > 0 ? hex.decode(v.key) : undefined;
        var h = new blake2xs_1.BLAKE2Xs(v.out.length / 2, { key: key });
        h.update(inp);
        ...
        var got = bytes_1.concat.apply(void 0, chunks);
        expect(hex.encode(got, true)).toEqual(v.out);
    });
    it("should generate digest with no message", function () {
        var h = new blake2xs_1.BLAKE2Xs(32);
        expect(h.digest().length).toBe(32);
    });
    it("should generate 65535 bytes", function () {
        var h = new blake2xs_1.BLAKE2Xs();
        h.update(new Uint8Array([1, 2, 3]));
        expect(h.digest().length).toBe(65535);
    });
    it("should throw when asked for more than 65535 bytes", function () {
        expect(function () { return new blake2xs_1.BLAKE2Xs(65536); }).toThrowError(/incorrect digest length/);
    });
    it("should throw when asked for more bytes than generated", function () {
        var h = new blake2xs_1.BLAKE2Xs(35);
        h.update(new Uint8Array([1, 2, 3]));
        h.stream(new Uint8Array(35));
        expect(function () { return h.stream(new Uint8Array(1)); }).toThrowError(/cannot/);
    });
    it("should throw when generating for more than 65535 bytes with unknown digest length", function () {
        var h = new blake2xs_1.BLAKE2Xs();
        h.update(new Uint8Array([1, 2, 3]));
        expect(function () { return h.stream(new Uint8Array(65536)); }).toThrowError(/cannot/);
    });
});
//# sourceMappingURL=blake2xs.test.js.map